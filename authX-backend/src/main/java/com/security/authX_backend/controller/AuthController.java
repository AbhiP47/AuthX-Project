...
import com.security.authX_backend.entity.RefreshTokenrequest;
...
public class AuthController {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<TokenResponse> refresh(@RequestBody(required = false)RefreshTokenrequest refreshTokenRequest, HttpServletResponse response , HttpServletRequest request) {
        ...
        RefreshToken storedRefreshToken = refreshTokenRepository.findByJwtId(jwtId).orElseThrow(()-> new BadCredentialsException("Invalid Refresh Token"));
        ...
        String newRefreshToken = jwtService.generateRefreshToken(user, newRefreshTokenOb.getJwtId());


        cookieService.attachRefreshCookie(response, newRefreshToken, (int) jwtService.getRefreshTtlSeconds());;
        return ResponseEntity.ok(TokenResponse.of(newAccessToken, newRefreshToken, jwtService.getAccessTtlSeconds(), "Bearer", modelMapper.map(user, UserDto.class)));
    }
        ...
        readRefreshTokenFromRequest(null, request).ifPresent(token -> {
            try {
                if (jwtService.isRefreshToken(token)) {
                    ...
                    refreshTokenRepository.findByJwtId(jti).ifPresent(rt -> {
                        rt.setRevoked(true);
...
//            1. prefer reading refresh token from cookie
        if (request.getCookies() != null) {
            ...
            Optional<String> fromCookie = Arrays.stream(request.getCookies())
                    .filter(c -> cookieService.getRefreshTokienCookieName().equals(c.getName()))
                    .map(Cookie::getValue)
                    .filter(v -> !v.isBlank())
                    .findFirst();
